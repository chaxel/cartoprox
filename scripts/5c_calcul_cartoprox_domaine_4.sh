...
#Selection du pas de temps regional
case ${polluant} in
0)polList="${var_traceur}";prefix_nc="_traceur";;
1)liste_var="O3 NO NO2";prefix_nc="_nox";;
2)liste_var="${var_pm}";prefix_nc="_${var_pm}";;
3)liste_var="${var_pm}";prefix_nc="_${var_pm}";;
esac

#Fichiers SIRANE/CARTOPROX
fic_sirane_nc=${ChemRes}/sirane${prefix_nc}.${run}.nc   #sirane_*.nc
fic_carto_nc=${ChemRes}/cartoprox${prefix_nc}.${run}.nc #cartoprox_*.nc

#Fichier REGIONAL
if [ ${drop_output_reg} -eq 1 ] ; then
  fic_reg_nc=${cartoprox_scratch}/reg${prefix_nc}.${code_domaine}.${run}.recepts.nc
else
  fic_reg_nc=${ChemRes}/reg${prefix_nc}.${run}.recepts.nc
fi

#Fichier UREMIS
if [ ${drop_output_suremis} -eq 1 ] ; then
  fic_sur_nc=${cartoprox_scratch}/suremis${prefix_nc}.${code_domaine}.${run}.recepts.nc
else
...
if [ -f ${ifnout} ] ; then
  ...
  for var in ${liste_var} ; do
    ...
    #version 4
    fond_fic_maille_tmp=${cartoprox_scratch}/`basename ${fond_fic_maille}.${code_domaine}.${var}`
...
#Formatage dates   
case ${var} in
PM10)  #Pour les PM10 utilise un fichier PREVALP journalier
