...
if [ ! -f ${fic_reg_nc} ] ; then
  ...
  time ncks -O -v Times,easting_pts,northing_pts -o ${fic_reg_nc} ${fic_sirane_nc}
  if [ ! -f ${ifnout} ] ; then
...
if [ -f ${ifnout} ] ; then
  for var in ${liste_var} ; do
    ...
    syntaxe="${extract_val_grille_exe} -i ${fond_fic_maille_tmp} -xmin ${xminc} -ymin ${yminc} -dx ${dxc} -s ${ifnout} -var ${var} ${ijour}" 
    echo $syntaxe
    time ${syntaxe} \
