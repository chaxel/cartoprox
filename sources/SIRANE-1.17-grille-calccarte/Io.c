...
/*---------------------------------*/
{
  ...
  for(i=0;i<Don.N_Rue;i++){
      fscanf(name,"%s %s %s %lf %lf %d %d",
	     R[i].Id,R[i].IdDeb,R[i].IdFin,
	     &R[i].H,&R[i].W,&R[i].Categ,&R[i].Autoroute);
  ...
  for(i=0;i<Don.N_Temps;i++){
    fscanf(name,"%lf/%lf/%lf %lf:%lf %lf %lf %s",&jour,
	   &mois,&annee,&heure,&minute,&Ite[i].Mod_rue_centre,&Ite[i].Mod_rue_autoroute,Ite[i].name_emisrue);
...
/*----------------------------*/
{
  ...
  for(i=0;i<nr;i++){
	...
	if ((*R)[j].Autoroute==1){
	  (*R)[j].Qsrce+=Iter.Mod_rue_autoroute*Qsrce;
	}
	else if ((*R)[j].Autoroute==0){
	  (*R)[j].Qsrce+=Iter.Mod_rue_centre*Qsrce;
	}
	else {Erreur("type de rue incohérent",1);}
	trouve=1;
  ...
  for(i=0;i<nr;i++){
    ...
    if(trouve==0) Erreur("Emission d'une source ponctuelle non affectee",1);
  }
  FermeFichier(name);
}
