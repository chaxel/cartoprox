...
/*---------------------------------------------*/
{
  ...
  DBL **Csurf_tot,**Csurf_tot_NO,**Csurf_tot_NO2,**Csurf_tot_O3;
  DBL x,y,xv,yv,**IndRN;
  Noeud *Nd;
  Rue *R;
  ...
  printf("**                                            **\n");
  printf("**         MODELE SIRANE VERSION 1.15         **\n");
  ...
  if(Don.calc_disp==1){
    ...
    if(Don.champ>=1){
      ...
      Csurf_tot=AllocMtrx(Grd.Nx,Grd.Ny);
	...
	Csurf_tot_NO=AllocMtrx(Grd.Nx,Grd.Ny);
	Csurf_tot_NO2=AllocMtrx(Grd.Nx,Grd.Ny);
	Csurf_tot_O3=AllocMtrx(Grd.Nx,Grd.Ny);
      }
    }
  }
  ...
  for(i=0;i<Don.N_Temps;i++){
	...
	if(Don.champ>=1){
	  ...
	  if(Don.Type_Pol!=1){
	    if (Don.grille_horaire==1) {
	      Ecrire_grd(Csurf,Grd,"",i+1);
	    }
	  }
	  ...
	  for(j=0;j<Grd.Nx;j++){
	    for(k=0;k<Grd.Ny;k++){
	      Csurf_tot[j][k]+=Csurf[j][k];
	      if(Don.Type_Pol==1){
		Csurf_tot_NO[j][k]+=Csurf_NO[j][k];
		Csurf_tot_NO2[j][k]+=Csurf_NO2[j][k];
		Csurf_tot_O3[j][k]+=Csurf_O3[j][k];
	      }
	    }
	  }	 
	}
  ...
  /*---------------------------------------------------*/
  /* calcul de la concentration moyenne sur la période */
  /* et ecriture des résultats                         */
  /*---------------------------------------------------*/
  
  /* Comptage des pas de temps valides */
                     
  for(k=0;k<Don.N_Temps;k++){
    if(Iter[k].valid==1) N_valid+=1;
  }
  /* calcul de la concentration moyenne */
  if (Don.champ>=1){
    for(j=0;j<Grd.Nx;j++){
      for(k=0;k<Grd.Ny;k++){
	Csurf_tot[j][k]=Csurf_tot[j][k]/N_valid;
	if(Don.Type_Pol==1){
	  Csurf_tot_NO[j][k]=Csurf_tot_NO[j][k]/N_valid;
	  Csurf_tot_NO2[j][k]=Csurf_tot_NO2[j][k]/N_valid;
	  Csurf_tot_O3[j][k]=Csurf_tot_O3[j][k]/N_valid;
	}
      }
    }
    /* ecriture des résultats*/
    if(Don.Type_Pol!=1) Ecrire_grd(Csurf_tot,Grd,"moy",0);
    else if(Don.Type_Pol==1){
      Ecrire_grd(Csurf_tot,Grd,"NOx_moy",0);
      Ecrire_grd(Csurf_tot_NO,Grd,"NO_moy",0);
      Ecrire_grd(Csurf_tot_NO2,Grd,"NO2_moy",0);
      Ecrire_grd(Csurf_tot_O3,Grd,"O3_moy",0);
    }
  }
  ...
  for(k=0;k<Don.grd.Nx*Don.grd.Ny;k++){
    free(Don.cell[k].Num_Infl);
    if(Don.cell[k].N_Rue>0)
      free(Don.cell[k].NumRue);
    if(Don.cell[k].N_Noeud>0)
      free(Don.cell[k].NumNd);
  }
  free(Don.cell);
  free(Don.Rec);
  free(Don.Percentiles);
  free(Don.Seuils);

  if(Don.Type_Pol==1){
    free(Don.Percentiles_NO);
    free(Don.Percentiles_NO2);
    free(Don.Percentiles_O3);
    free(Don.Seuils_NO);
    free(Don.Seuils_NO2);
    free(Don.Seuils_O3);
  }
  ...
  if(Don.calc_disp==1){
    ...
    if(Don.champ>=1){
      ...
      LibereMtrx(Csurf_tot,Grd.Nx,Grd.Ny);
      if(Don.Type_Pol==1){
	LibereMtrx(Csurf_NO,Grd.Nx,Grd.Ny);	
	LibereMtrx(Csurf_NO2,Grd.Nx,Grd.Ny);	
	LibereMtrx(Csurf_O3,Grd.Nx,Grd.Ny);
  LibereMtrx(Csurf_tot_NO,Grd.Nx,Grd.Ny);	
	LibereMtrx(Csurf_tot_NO2,Grd.Nx,Grd.Ny);	
	LibereMtrx(Csurf_tot_O3,Grd.Nx,Grd.Ny);		
      }
    }
  }
