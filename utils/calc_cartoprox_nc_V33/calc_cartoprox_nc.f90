...
!3.2 toutes les variaables ont le même nom
      ...
      integer :: ncSiraneID, ncRegID, ncSurID  , ncCartoID    
      ...
      character(len=20) :: varstr_suremis
...
!recherche du point le plus proche
	...
	if (ip.lt.nprint) write(*,'("PT ",I6,F9.1,F10.1,F10.1,F10.1)') ip, xpt(ip), ypt(ip), dist(ip), area(ip)  
      ...
      call check(nf90_open(fic_carto, nf90_write  , ncCartoID))          
      if ( nf90_inq_varid(ncCartoID, 'area_pts', xVarID) .eq. nf90_noerr ) &
        call check(nf90_put_var(ncCartoID, xVarID, area, count=(/numPoints/), start=(/1/) ) )      
      call check(nf90_close(ncCartoID))
...
!-------------------------------------------------------------------------------------------
      ...
      if (nf90_open(fic_carto,  nf90_write    ,ncCartoID).ne.nf90_noerr)stop 'STOP: Erreur ouverture fichier CARTOPROX'         
!-------------------------------------------------------------------------------------------     
!Ouvre les fichiers
!-------------------------------------------------------------------------------------------      
      do iv = 1, nvar      
	...
	varstr_cartoprox = trim(adjustl(varstr(iv)))
        ...
        varstr_suremis   = trim(adjustl(varstr(iv)))
	...
	iprox=.false.
        ...
        if ( nf90_inq_varid(ncSurID,    varstr_suremis,   xVarID) .eq. nf90_noerr)iprox=.true.  
	  ...
	  isirane=.true.	   
          if (.not.(ifond.and.igrille.and.iprox.and.isirane)) then        
            ...
            write(*,*) 'suremission ('//trim(varstr_suremis)//')=', iprox	    
...
!-------------------------------------------------------------------------------------------                                   	  	           
	...
	call check(nf90_inq_varid(ncCartoID, 'Times', xVarID) )   
        call check(nf90_put_var(ncCartoID, xVarID, datestr, start=start2d ) )
	...
	if(idebug)write(*,*)  	datestr  
	  ...
	  if ( nf90_inq_varid(ncSiraneID, varstr_sirane, xVarID) .eq. nf90_noerr) then
	   ...
	   if (idebug)write(*,*)'Lit '//trim(varstr_suremis)//' dans '//trim(fic_sur)
           call check(nf90_inq_varid(ncSurID, varstr_suremis, xVarID) )
	...
	do iv = 1, nvar
	   ...
	   if ( index(varstr_sirane,'NO')+index(varstr_sirane,'O3').ne.0) then
             write(*,*)'Creer NO, NO2 & O3 dans '//trim(fic_carto)//' ?'
	     call create_var_pt_nc(ncCartoID,'NO',xVarID)
	     call create_var_pt_nc(ncCartoID,'NO2',xVarID)
	     call create_var_pt_nc(ncCartoID,'O3',xVarID)
	   end if
	   
	   !PM
	   if ( index(varstr_sirane,'PM').ne.0) then
	     write(*,*)'Creer '//trim(varstr_sirane)//' dans '//trim(fic_carto)
	     call create_var_pt_nc(ncCartoID,varstr_sirane,xVarID)
	   end if
	   ...
	   !Ecrit dans le fichier CARTOPROX	
           if ( nf90_inq_varid(ncCartoID, varstr_cartoprox, xVarID) .eq. nf90_noerr ) then
	     if(idebug)&
	     write(*,*)'Ecrit  '//trim(varstr_cartoprox)
	     call check(nf90_put_var(ncCartoID, xVarID, conc_cartoprox(iv,:), count=(/numPoints/), start=start2d ) )
           end if	   
