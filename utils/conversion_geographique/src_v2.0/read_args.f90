...
! DONNEE GEOGRAPHIQUES
	  ...
	  if ((.not.il93_i).and.(.not.il2_i).and.(.not.iutm_i).and.(.not.igeo_i) )  then 
	    il2_i = .true.
	  else 
	    il2_o = .true.
	  end if
	  
        else if ( trim(adjustl(argstr)).eq. '-l93' )  then	  
	  if ((.not.il93_i).and.(.not.il2_i).and.(.not.iutm_i).and.(.not.igeo_i) )  then 
	    il93_i = .true.
	  else 
	    il93_o = .true.
	  ...
	  if (     ((.not.il93_o).and.(.not.il2_o).and.(.not.iutm_o).and.(.not.igeo_o)) &
	      .and.( il93_i.or.il2_i.or.iutm_i.or.igeo_i                              ) ) then		  
	    call getarg(iarg+1,argstr)
	...
	goto 99
      ...
      write(*,*) 'Syntaxe  -i fichier.XY [-utm 31 -l2 -l93 -geo -xc -yc -dx -polair]'  
...
! ENTREES
!---------------------------------------------------------------------
      if ((.not.il93_i).and.(.not.iutm_i).and.(.not.igeo_i).and.(.not.il2_i)) then
        write(*,*) 'indiquer le systeme entree: -geo -l2 -l93 -utm'
	stop
      end if
      ...
      !Regles speciales
      if (il93_i)geoidstr_i = 'GRS80'
      if (il2_i )geoidstr_i = 'NTF'      
...
! SORTIES
!---------------------------------------------------------------------
      if ((.not.il93_o).and.(.not.iutm_o).and.(.not.igeo_o).and.(.not.il2_o)) then
        write(*,*) 'indiquer le systeme de sortie: -geo -l2 -l93 -utm'
	stop
      end if 
      ...
      !Regles speciales
      if (il93_o)geoidstr_o = 'GRS80'
      if (il2_o )geoidstr_o = 'NTF' 
...
!--------------------------------------------------------------------
      ...
      write(*,*) '#######################################################################'
      write(*,*) ''
      write(*,*) 'BUT: Conversion de coordonnees geographique, UTM et Lambert 93 et Lambert 2 etendu'
