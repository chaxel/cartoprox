...
! VERSION 5 : utilise un fichier de stations au format NetCDF
! VERSION 6 : optimisation sur les dates, utilise datestr1 et datestr2
...
! Allocations
      ...
      !Determernine itstart et itstop
	 ...
	 if(datestr.eq.datestr1)itstart=it
	 if(datestr.eq.datestr2)then
	   itstop=it
	   go to 96 !OPTIMISATION
	 end if	 	 	 
	end do
96      continue
	if(itstart.eq.0)write(*,*)'Ne trouve pas la date'//datestr1//' dans le fichier'//trim(fic_grille_nc)
	if(itstop .eq.0)write(*,*)'Ne trouve pas la date'//datestr2//' dans le fichier'//trim(fic_grille_nc)
	if(itstart*itstop.eq.0)stop 
	!Version 6 ---------------------------------------------------------------------------------------------
      else
        itstart=1
        itstop=numsteps      
