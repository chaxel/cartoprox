...
! DONNEE GEOGRAPHIQUES
	...
	stop
      ...
      if (ixc.and.iyc.and.(idx.or.idy)) then
        igrilleuser=.true. 
        write(*,'("Centre domaine X=",F12.3)') xc 
        write(*,'("Centre domaine X=",F12.3)') yc 
        write(*,'("Largeur du domaine DX=",F12.3)') dx 
        write(*,'("Hauteur du domaine DY=",F12.3)') dy			     
      else
        write(*,*) '-> Grille AUTO active'        
      end if	
      
      if (ibrin_dx.and.irecept_dx.and.imif) then ! toutes les entrees sont OK
        write(*,'("Largeur de discretisation des brins (m)=",F7.0)') brin_dx
        write(*,'("Pas de la grille de depart (m)=",F7.0)') dx_recept_ini  
        write(*,'("Largeur du cadre (m)=",F7.0)') cadre_dx
        go to 99   
