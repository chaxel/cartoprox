...
!     arguments
	...
	iarg = iarg + 1	
	  ...
	  read(argstr,*) yc
	  iyc = .true.
        else if (( trim(adjustl(argstr)).eq. '-fac_geometrie' ).or.&
	         ( trim(adjustl(argstr)).eq. '-fac_geom'      ).or.&
		 ( trim(adjustl(argstr)).eq. '-fc'            ) )  then
	  call getarg(iarg+1,argstr)
	  read(argstr,*) facteur_geom
	  ifc = .true.	  
	  ...
	  irecept_dx = .true.
        else if ( trim(adjustl(argstr)).eq. '-max_dx' )  then	  
	  call getarg(iarg+1,argstr)
	  read(argstr,*) dy_recept_max
...
! DONNEE GEOGRAPHIQUES
	...
	stop
      ...
      !valeur par defaut
      if (.not.ifc) then
        write(*,*) '-> Facteur de grille DEFAUT:',facteur_geom_defaut
        facteur_geom = facteur_geom_defaut
      else
        write(*,*) '-> Facteur de grille USER:',facteur_geom
      endif

      !valeur par defaut
      !if (.not.inc)nclass_recept = nclass_recept_defaut
      ...
      if (ibrin_dx.and.irecept_dx.and.imif) then ! toutes les entrees sont OK
        ...
        write(*,'("Pas de la grille de depart (m)=",F7.0)') dx_recept_ini  
      ...
      write(*,*) 'Syntaxe -optimal[-virtuel] -i fichier.MIF -brin_dx -recept_dx -cadre_dx [-utm 31 -xc -yc -dx -fc -nc]'  
...
!--------------------------------------------------------------------
      ...
      write(*,*) ''      
      write(*,*) 'SYNTAXE : mailleur.e -optimal[-virtuel] -i fichier.MIF -brin_dx -recept_dx -max_dx -cadre_dx -g grille.txt'  
      write(*,*) ''	   
      write(*,*) 'ENTREE  : -i fichier.MIF        = fichier de brins au format MIF'
      write(*,*) '          -brin_dx brin_dx      = longueur de discretisation des brins (m)'
      write(*,*) '          -recept_dx recept_dx  = pas de la grille de depart (m)'
      write(*,*) '          -max_dx max_dx        = distance max. entre 2 recepteurs (m)'      
      write(*,*) '          -cadre_dx cadre_dx    = largeur du cadre autour du domaine (m)'
      write(*,*) '          -nc nb_recept         = nombre de classes de distance'      
      write(*,*) '          -fc facteur_recept    = facteur multiplicatif entre 2 classes'      
