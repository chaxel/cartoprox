...
98        continue	       	       
	  ...
	  if (itime) var_dim = var_dim - 1 ! le temps est une dimension
	  
	  nlev = 1
	  if (itime) then
            if (var_dim.eq.3)call check(nf90_get_var(ncFileID, xVarID, var_data(:,:,1) ,  start=(/1,1,1,it/),count=(/nxx,nyy,nlev/))) 
            if (var_dim.eq.2)call check(nf90_get_var(ncFileID, xVarID, var_data(:,:,1) ,  start=(/1,1,  it/),count=(/nxx,nyy,   1/))) 
	  else
	 ...
	 do is =1, nstat
      ...
      if(ihour)write(*,'(A19,F10.2)') datestr,conc_tmp
