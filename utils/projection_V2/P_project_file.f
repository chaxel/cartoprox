...
C in one specific geodesic system. It uses USGS algorithms.
	...
	character(len=10) projection(3) !Projections
...
C-------Some arrays
	...
	ref(4)='GRS80'	
	ref(5)='USER'
	projection(1)='LambertCC '
	projection(2)='UTM       '
	projection(3)='Geographic'
	units(1)='metres    '
...
1       continue
	...
	if (proj1.eq.3) proj1=0 !PROJ=0 FOR GEOGRAPHIC
	...
	if (transfo.EQ.1) then
	 ...
	 gs1=3 !NTF
	 gs2=1 !WGS84
	else if (transfo.EQ.5) then
	 gs1=2 !ED50
	 gs2=3 !NTF
	else if (transfo.EQ.6) then
	 gs1=3 !NTF
	 gs2=2 !ED50
...
c	if (proj1.eq.0) then
c	  write(*,*) 'Enter LONGITUDE '//ref(gs1)//' (in degrees)'
c	  read(*,*) x1
c	  write(*,*) 'Enter LATITUDE '//ref(gs1)//' (in degrees):'
c	  read(*,*) y1
c     ****Degrees -> Radians***
c	  x1=x1*D2R
c	  y1=y1*D2R
c	else
c	  write(*,*) 'projection ',projection(proj1)
c	  write(*,*) 'Enter X_'//ref(gs1)//
c     &           ' (in '//trim(units(int(div)))//'):'
c	  read(*,*) x1
c	  write(*,*) 'Enter Y_'//ref(gs1)//
c     &           ' (in '//trim(units(int(div)))//'):'
c	  read(*,*) y1
c	end if
c     ****Is it UTM ?***
...
c c     write(*,*) '***********RESULT************'
c	write(*,*) 'System= '//ref(gs1)
c      if (proj1.eq.0) then
c	  write(*,'(" LON=",f11.6," degrees")') x1*R2D
c	  write(*,'(" LAT=",f11.6," degrees")') y1*R2D
c	else
c	 write(*,*) 'projection ',projection(proj1),int(zone)
c	 write(*,'(" X=",f13.3,1X,A10)') x1,units(int(div))
c	 write(*,'(" Y=",f13.3,1X,A10)') y1,units(int(div))       
c	end if

        open(unit=10,file='coord1.txt',status='old')
        open(unit=11,file='coord2.txt',status='unknown')	
...
c     ****DISPLAY RESULTS OF 2ND TRANSFORMATION****
	...
	write(*,*) 'System= '//ref(gs2)
	 ...
	 write(*,*) 'projection ',projection(proj2),int(zone)
