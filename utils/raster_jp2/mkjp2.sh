...
echo "generation du raster jp2" | tee -a ${log}
...
echo ${cmd} >>${log}
${cmd} 1>>${log} 2>>${log}
[ $? -ne 0 ] && { echo "erreur dans la creation de l'image coloree"; exit 1; }
echo >>${log}

cmd="convert ${tmpdir}/img.tif -flip ${tmpdir}/img.flip.tif"
echo ${cmd} >>${log}
${cmd} 1>>${log} 2>>${log}
[ $? -ne 0 ] && { echo "erreur dans la correction Y de l'image coloree"; exit 1; }
echo >>${log}

cmd="gdal_translate -a_ullr ${xmin} ${ymax} ${xmax} ${ymin} -a_srs EPSG:${epsg} -of GTiff ${tmpdir}/img.flip.tif ${tmpdir}/raster.tif"
echo ${cmd} >>${log}
${cmd} 1>>${log} 2>>${log}
[ $? -ne 0 ] && { echo "erreur dans la creation du raster !"; exit 1; }
echo >>${log}

#cmd="${kdu_home}/kdu_compress -i ${tmpdir}/raster.tif -o ${fno}"
cmd="mv ${tmpdir}/raster.tif ${fno}"
echo ${cmd} >>${log}
${cmd} 1>>${log} 2>>${log}
[ $? -ne 0 ] && { echo "erreur dans la creation du raster jp2"; exit 1; }
echo >>${log}


rm -rf ${tmpdir}
echo "-> creation du raster ${fno}" | tee -a ${log}
exit 0
