...
## Création d'un raster de valeur au format jpg2000 ##
## N'utilise pas kakadu -> erreur
...
for cdf in ${cdfs}; do
	...
	# Création d'un raster à partir des données
	cmd="python nc2raster.py ${var} ${cdf} ${tmpdir}/raster_${xc}_${yc}_${var}.tif"
	echo ${cmd} >>${log}
	${cmd} 1>>${log} 2>>${log}
	[ $? -ne 0 ] && { echo "erreur dans la création du raster raster_${xc}_${yc}_${var}.tif !"; exit 1; }
	echo >>${log}

	# Géoréférencement du raster
	cmd="gdal_translate -a_ullr ${xmin} ${ymax} ${xmax} ${ymin} -a_srs EPSG:${epsg} -of GTiff ${tmpdir}/raster_${xc}_${yc}_${var}.tif ${tmpdir}/raster_geo_${xc}_${yc}_${var}.tif"
	echo ${cmd} >>${log}
	${cmd} 1>>${log} 2>>${log}
	[ $? -ne 0 ] && { echo "erreur dans la création du raster raster_geo_${xc}_${yc}_${var}.tif !"; exit 1; }
...
cmd="gdal_merge.py -o ${tmpdir}/raster_merge_${var}.tif -of GTiff -n -9999 ${tmpdir}/raster_geo*.tif" 
echo ${cmd} >>${log}
${cmd} 1>>${log} 2>>${log}
...
# Compression en jp2 ## ERROR Kakadu
...
cp -v ${tmpdir}/raster_merge_${var}.tif ${out}.tif
...
echo "  File \"`basename ${out}`.tif\"" >>${tab}
