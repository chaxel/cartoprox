...
!-------------------------------------------------------------------------------------
   subroutine ecrit_an_statnc_grille
      ...
      call check(nf90_put_var(statfileID, XVarID, no2_moy_an, start = (/ 1,1/) ))
...
!-------------------------------------------------------------------------------------
   ...
   subroutine ecrit_mois_ncstat_grille
	 ...
	 call check(nf90_inq_varid(statfileID, 'no2_moy_mois', xVarID))      
         call check(nf90_put_var(statfileID, XVarID, no2_moy_mois, start = (/ 1,1,mois_en_cours/) ))   
	 if (ipm) then
   ...
   subroutine ecrit_jour_ncstat_grille
	...
	do j1=1,ny1	      		 
	  ...
	  call check(nf90_inq_varid(statfileID, 'no2_max_1hr_jour', xVarID))      
          call check(nf90_put_var(statfileID, XVarID, no2_max_1hr_jour, start = (/ 1,1,it_jour/) ))	  	
...
!-------------------------------------------------------------------------------------
   subroutine lit_step_ncstat_grille
      ...
      ! O3 KRIG
      if ( nf90_inq_varid(out1fileID, 'O3_KRIG'  , xVarID) .eq. nf90_noerr ) o3_var   ='O3_KRIG'
      if ( nf90_inq_varid(out1fileID, 'NO2_KRIG' , xVarID) .eq. nf90_noerr ) no2_var  ='NO2_KRIG'  
      if ( nf90_inq_varid(out1fileID, 'NO_KRIG'  , xVarID) .eq. nf90_noerr ) no_var   ='NO_KRIG'
      if ( nf90_inq_varid(out1fileID, 'PM10_KRIG', xVarID) .eq. nf90_noerr ) pm10_var ='PM10_KRIG'
