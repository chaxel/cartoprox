...
!-------------------------------------------------------------------------------------
   subroutine ecrit_an_statnc_point
      ...
      call check(nf90_put_var(statfileID, XVarID, no2_moy_an, start = (/ 1/) ))
   ...
   subroutine ecrit_mois_ncstat_point
	 ...
	 if (igaz) then !GAZ 
	   ...
	   call check(nf90_inq_varid(statfileID, 'no2_moy_mois', xVarID))      
           call check(nf90_put_var(statfileID, XVarID, no2_moy_mois, start = (/ 1,mois_en_cours/) ))   
	 end if
	...
	do j1=1,ny1	      		 
	  ...
	  if (igaz) then !GAZ 
	    ...
	    call check(nf90_inq_varid(statfileID, 'no2_max_1hr_jour', xVarID))      
            call check(nf90_put_var(statfileID, XVarID, no2_max_1hr_jour, start = (/ 1,it_jour/) ))	  	
	  end if 
...
!-------------------------------------------------------------------------------------
   subroutine lit_step_ncstat_point
      ...
      if (igaz) then  
        ...
        call check(nf90_get_var(out1fileID, xVarID, no2_0,  start=start2d ))   
      end if !GAZ
