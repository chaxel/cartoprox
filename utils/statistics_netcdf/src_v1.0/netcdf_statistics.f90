...
! gaz Y/N
      write(*,*) 'Verifie la presence de NO2/PM10'    
...
!--------------------------------------------------------------------            
	...
	tem2_max_1hr_jour = -9999.
	 ...
	 n_o3_moy_mois   = 0.
      ...
      if ( it .eq. 1 ) then
        if (ipm) write(*,'(A10,2A10,4A10)') 'Date' ,'AOT_foret','AOT_veget','O3 max','NO2 max','PM10 max','ecart max'
	if (.not.ipm) write(*,'(A10,2A10,2A10)') 'Date' ,'AOT_foret','AOT_veget','O3 max','NO2 max'
      end if
      ...
      if (  ijour_new.or.(it.eq.1) ) then
        if (iaot_foret.and.(.not.iaot_veg)) then
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),'X',' ',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1)),maxval(pm10_0(:,:,1)),maxval(pm10_ecart_0(:,:,1)),correction_pm10_jour
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),'X',' ',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1))
	else if (iaot_foret.and.iaot_veg) then
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),'X','X',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1)),maxval(pm10_0(:,:,1)),maxval(pm10_ecart_0(:,:,1)),correction_pm10_jour 
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),'X','X',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1))
        else 	
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),' ',' ',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1)),maxval(pm10_0(:,:,1)),maxval(pm10_ecart_0(:,:,1)),correction_pm10_jour
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),' ',' ',maxval(o3_0(:,:,1)),maxval(no2_0(:,:,1))
        end if
      end if               
	...
	!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      !--------------------------------------------------------------- 
	   ...
	   pm10_0(i1,j1,1) = pm10_0(i1,j1,1)  + correction_pm10_jour
	   if (pm10_0(i1,j1,1).lt.0.)pm10_0(i1,j1,1)=0.
	   
	   ! moyennes jour
	   pm10_moy_jour(i1,j1)      = pm10_moy_jour(i1,j1)       + pm10_0(i1,j1,1)
	   pm10_ecart_moy_jour(i1,j1)= pm10_ecart_moy_jour(i1,j1) + pm10_ecart_0(i1,j1,1)
           ! moyennes an/mois
