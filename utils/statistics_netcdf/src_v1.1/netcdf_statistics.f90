...
! gaz Y/N
      ...
      if ( (nf90_inq_varid(out1fileID, 'O3' , xVarID).eq.nf90_noerr).and. &
           (nf90_inq_varid(out1fileID, 'NO2', xVarID).eq.nf90_noerr).and. &
           (nf90_inq_varid(out1fileID, 'NO' , xVarID).eq.nf90_noerr) ) then
        write(*,*) '> GAZ O3/NO2/NO DISPONIBLES'
	igaz=.true.
      else
        write(*,*) '> GAZ O3/NO2/NO INDISPONIBLES'      
        igaz=.false.      
      end if 
	...
	if (.not.ipm) write(*,'(A10,2A10,2A10)') 'Date' ,'AOT_foret','AOT_veget','O3 max','NO2 max'
      ...
      if (  ijour_new.or.(it.eq.1) ) then
        if (iaot_foret.and.(.not.iaot_veg)) then
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),'X',' ',maxval(o3_0(:,:)),maxval(nox_0(1,:,:)),maxval(pm10_0(:,:)),maxval(pm10_ecart_0(:,:)),correction_pm10_jour
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),'X',' ',maxval(o3_0(:,:)),maxval(nox_0(1,:,:))
	else if (iaot_foret.and.iaot_veg) then
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),'X','X',maxval(o3_0(:,:)),maxval(nox_0(1,:,:)),maxval(pm10_0(:,:)),maxval(pm10_ecart_0(:,:)),correction_pm10_jour 
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),'X','X',maxval(o3_0(:,:)),maxval(nox_0(1,:,:))
        else 	
           if (ipm)     write(*,'(A13,2(5X,A1,4X),4F10.2," correction PM10",F7.2)') datestr(1:13),' ',' ',maxval(o3_0(:,:)),maxval(nox_0(1,:,:)),maxval(pm10_0(:,:)),maxval(pm10_ecart_0(:,:)),correction_pm10_jour
	   if (.not.ipm)write(*,'(A13,2(5X,A1,4X),2F10.2)') datestr(1:13),' ',' ',maxval(o3_0(:,:)),maxval(nox_0(1,:,:))
        end if
      end if               
