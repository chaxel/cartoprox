...
!-------------------------------------------------------------------------------------
   subroutine create_ncstat_point
    ...
    call check(nf90_def_var(statfileID,'pm25_moy_an'  ,nf90_float,dimids=(/pointDimID/),varID=XVarID))    
...
!-------------------------------------------------------------------------------------
   subroutine ecrit_an_statnc_point
      ...
      call check(nf90_put_var(statfileID, XVarID, pm25_moy_an, start = (/ 1/) ))    
...
!-------------------------------------------------------------------------------------
   subroutine ecrit_jour_ncstat_point
	...
	pm10_moy_jour(i1,j1) = pm10_moy_jour(i1,j1) / n_pm10_moy_jour(i1,j1)
        ...
        ! --- PM25: jour de depassement du x ug/m3
        if (ipm25) then
...
!-------------------------------------------------------------------------------------
   subroutine lit_step_ncstat_point
	...
	! PM2.5
        ...
        call check(nf90_get_var(out1fileID, xVarID, pm25_0,  start=start2d ))	 
      end if ! ipm 
