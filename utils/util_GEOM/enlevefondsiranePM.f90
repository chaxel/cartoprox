...
real, dimension(:),allocatable :: fNOx,fNO,fNO2,fO3,fPM
real, dimension(:),allocatable :: NOx,NO,NO2,O3,PM
real, dimension(:),allocatable :: x,y,inum
real, dimension(:,:),allocatable :: outNOx,outNO,outNO2,outO3,outPM
...
integer :: varidno,varidno2,varido3,varidnox,varidpm
...
!     fnif="fond_2008.dat.4f90" 
      ...
      rewind(ifrf)
      do i=1,nbech
       read(ifrf,*)(fondbid(j,i),j=1,4),fPM(i)
!       fNOx(i)=46.*fNO(i)/30.+fNO2(i)
      enddo !i
       i=10
      print*,'fond ech 10 : no no2 o3 ',  fPM(i)
      close(ifrf)
!
!..............
...
!      print*,'fni=',trim(fni),len(trim(crue))
      ...
      rewind(ifr)
        read(ifr,*)cbidon
        do i=1,nbech
          read(ifr,*)(valout(j,i),j=1,3),PM(i)
        enddo !i
      close(ifr)
!  i=10
...
call check( nf90_def_var(ncid, "PM", NF90_REAL, dimids, varidpm) )
...
call check( nf90_put_var(ncid, varidpm, outPM) )
